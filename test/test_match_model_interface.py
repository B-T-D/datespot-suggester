...
class TestHelloWorldThings(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.userKeyGrort = self.user_api.create(json.dumps({"name":grortName, "current_location": grortCurrentLocation, "force_key": "1"}))
        self.userKeyDrobb = self.user_api.create(json.dumps({"name":drobbName, "current_location": drobbCurrentLocation, "force_key": "2"}))
        ...
        self.userKeyMiltrudd = self.user_api.create(json.dumps({"name":"Miltrudd", "current_location":(41.0, -72.0), "force_key": "3"}))
        self.userMiltrudd = self.user_api.lookup_obj(self.userKeyMiltrudd)
        # match Grort with Miltrudd:
        self.knownMatchKey = self.api.create(json.dumps({"user1_id": self.userKeyGrort, "user2_id": self.userKeyMiltrudd}))
    ...
    def test_create(self):
        matchKey = self.api.create(json.dumps({"user1_id": self.userKeyGrort, "user2_id": self.userKeyDrobb}))
